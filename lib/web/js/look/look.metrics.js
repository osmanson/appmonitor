	...
	this.Metrics.add = function (buffer) {
		...
		buffer.forEach(function (metric) {
			...
			var replaceRegex = /\[|\]|\.|\/|\s|\:/g,
				...
				sourceTabId = 'tab-metrics-source-' + metric.source.replace(replaceRegex, ''),
				scopeSectionId = 'section-metrics-' + metric.source.replace(replaceRegex, '') + '-' + metric.scope.replace(replaceRegex, '-'),
				plotId = 'plot-metrics-' + metric.source.replace(replaceRegex, '') + '-' + metric.scope.replace(replaceRegex, '-') + '-' + metric.name.replace(replaceRegex, '-');
